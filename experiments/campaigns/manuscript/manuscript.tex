...
\begin{enumerate}
  ...
  \item \textbf{Surface binding}: every adsorbate \emph{molecule}
    ...
    \texttt{galoop.science.reproduce.\_group\_molecules}) has at least
    one atom within 1.3$\times$covalent-radii of a slab atom.
...
\begin{methodology}[Three observed GPR regimes]
\textbf{Helpful regime.} Pt(111)-ORR and Ni(111)-CH$_x$: 4--5 species,
enough composition dimensions for the Mat{\'e}rn kernel to learn a
meaningful surface, and GPR proposals steer directly into
saturation-level OH/OOH and CH$_2$/CH$_3$ mixtures that the GA
operators would only reach by many splice/merge cycles.\\[4pt]
\textbf{Irrelevant regime.} Cu(111) with CO/H/H$_2$O: the search space
is small enough that \texttt{init} placement alone already hit the
optimum before GPR collected its \texttt{gpr\_min\_samples}=20
training points. GPR activated but never contributed to the top 5.\\[4pt]
\textbf{Misled regime.} Pt(211)-NRR: the GPR is trained on species
counts that are reported by the (buggy) greedy
\texttt{infer\_adsorbate\_counts}, which over-credits NH$_3$ at the
expense of the NH$_x$ intermediates. GPR proposals inherit the bias
and cluster at ``apparently high NH$_3$'' compositions. The $G$ values
are still physically correct (CHE linearity absorbs the miscount),
but the GPR's composition map is wrong. See
...
\begin{table}[H]
\centering\small
\begin{tabular}{@{}clp{0.48\linewidth}l@{}}
\toprule
\# & area & description & status \\
\midrule
1  & \texttt{science/surface.py::\_surface\_normal} &
   Used $\hat c$ instead of $\widehat{\mathbf{a}\times\mathbf{b}}$;
   wrong on tilted-$c$ cells. &
   fixed \texttt{d5e5687}, regression test in \texttt{325c6ce} \\
4  & \texttt{loop.py} exit path &
   \texttt{parsl.clear()} called without \texttt{parsl.dfk().cleanup()};
   Python hung at the end of every clean run. &
   fixed \texttt{0a22b65} \\
9  & \texttt{spawn.py::spawn\_one} crossover bounds check &
   Enforced total \texttt{max\_adsorbates} but not per-species
   \texttt{max\_count}. Caught on Cu(211). &
   fixed \texttt{a6a0fb9} \\
10 & torch / torchvision / e3nn &
   \texttt{fairchem-core} install bumped torch 2.6$\to$2.8;
   torchvision 0.21 and e3nn pickling formats broke. &
   torchvision bumped; e3nn pinned \\
12 & \texttt{spawn.py::infer\_adsorbate\_counts} &
   Greedy formula-length miscount for element-overlapping species
   (O/OH/OOH/H, NH$_x$, CH$_x$). $G$ invariant under CHE linearity,
   reports unreliable. &
   \textbf{open} (Section~\ref{sec:limitations}) \\
--- & \texttt{engine/scheduler.py} parsl logs &
    Unbounded \texttt{manager.log} growth when workers outlive their
    parent. 30+~GB leaked across 18 orphaned workers during the
    sweep before it filled the disk. &
    fixed via \texttt{RotatingFileHandler} monkey-patch \\
\bottomrule
\end{tabular}
\caption{Code bugs surfaced and closed during the validation sweep.
Commit SHAs reference the \texttt{main} branch.}
\label{tab:bugs}
\end{table}
